...
public class Jeu {
    ...
    */
    ...
    public boolean collisionEntreAvatarEtFleur() {
        if ((fleur.getX() >= avatar.getX() + avatar.getLargeur()) // trop à droite
            || (fleur.getX() + fleur.getLargeur() <= avatar.getX()) // trop à gauche
            || (fleur.getY() >= avatar.getY() + avatar.getHauteur()) // trop en bas
            || (fleur.getY() + fleur.getHauteur() <= avatar.getY())) { // trop en haut
            return false;
        } 
        else {
            return true;
